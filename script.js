...
async function showNews() {
	...
	function newsBallon(query) {
		...
		fetch("http://newsapi.org/v2/top-headlines?country=us&category=" + query + "&apiKey=" + newsApi) 
	...
	recognition.onresult = (e) => {
		...
		//----------------------Youtube Videos
			...
			addAnswer(Main, "<a href=" + url + " target='_blank'>For more click below</a>", videoSearch(API_KEY, stringWithoutCommas, 10));
			...
			function weatherBalloon(cityName) {
				...
				fetch('https://api.openweathermap.org/data/2.5/weather?q=' + cityName + '&units=imperial&appid=' + key)
					...
					.then(function (data) {
						...
						addAnswer(Main, "The temperature is <br><b>" + temp + " F</b><br><b>" + disc + "</b><br><img src='" + iconurl + "'>");
						let utter4 = new SpeechSynthesisUtterance("The temperature is " + parseFloat((temp - 32) * 5 / 9).toFixed(2) + "degree celsius and climate is" + disc);
						synth.speak(utter4);
					})
					.catch(function () {
						// catch any errors
